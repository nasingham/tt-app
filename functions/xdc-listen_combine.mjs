...
export default async(request,context) => {
    ...
    const combinedStore = getStore("xdc");
    ...
    if (storedData && storedTimestamp && currentTime-storedTimestamp < CACHE_DURATION){
        ...
        try{
            ...
            const [fetch_deployments, fetch_titleCreated] = await Promise.all([
                // fetch('http://localhost:9999/.netlify/functions/xdc-listen_deployer'),
                // fetch('http://localhost:9999/.netlify/functions/xdc-listen_titleEscrow')
                fetch ('https://tradetrust-app.netlify.app/.netlify/functions/xdc-listen_deployer'),
                fetch ('https://tradetrust-app.netlify.app/.netlify/functions/xdc-listen_titleEscrow')
            ]);
