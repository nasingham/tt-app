...
export default async(request,context) => {
    ...
    const combinedStore = getStore("polygon");
    ...
    if (storedData && storedTimestamp && currentTime-storedTimestamp < CACHE_DURATION){
        ...
        try{
            ...
            const [fetch_deployments, fetch_titleCreated] = await Promise.all([
                // fetch('http://localhost:9999/.netlify/functions/polygon-listen_deployer'),
                // fetch('http://localhost:9999/.netlify/functions/polygon-listen_titleEscrow')
                fetch ('https://tradetrust-app.netlify.app/.netlify/functions/polygon-listen_deployer'),
                fetch ('https://tradetrust-app.netlify.app/.netlify/functions/polygon-listen_titleEscrow')
            ]);
