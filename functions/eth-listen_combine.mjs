...
export default async(request,context) => {
    try{
        const fetch_deployments = await fetch('http://localhost:8888/.netlify/functions/eth-listen_deployer');
        const fetch_titleCreated = await fetch('http://localhost:8888/.netlify/functions/eth-listen_titleEscrow');
